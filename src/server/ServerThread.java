...
public class ServerThread extends Thread {
    ...
    public static void envoyermail(String adrs) {
    	String mailscol = "projetsmtp4" ;
    	String mdp = "AZERTYUIOP1";
    	    Properties p = new Properties();
    	    p.put("mail.smtp.ssl.trust", "smtp.gmail.com");
    	    p.put("mail.smtp.ssl.protocols", "TLSv1.2");
    	    p.put("mail.smtp.port", "587");
    	    p.put("mail.smtp.host", "F.gmail.com");
    	    ...
    	    try {
    	      ...
    	      m.setSubject("inscription à TalkApp");
    	      m.setText("vouus etes desormais inscrit à TalkApp");
             // Transport transp = get ; 
    	      Transport.send(m);
             // char[] Adresse= adrs.toCharArray(); 
              //Transport.send(m, Adresse);
                        ...
                        case "CMD_PRIVATECHAT":
                          ...
                          /*  File publicKeyFile = new File("public.key");
                            ...
                            // crypting misty1key
                             ...
                             String encryptedMisty1Key = RSACipher.encrypt("10010101001", publicKey);
                            // cypting message with misty1
                            */ /* try{
                                userDB.insertMessage(messageContent,clientName, privateReceiver);
                            } catch(Exception e){
                          ...
                          /*  File publicKeyFile = new File("public.key");
                            ...
                            // cypting message with misty1
                            */
                            // reading the recievers public key in order to crypt misty1's key in it using RSA
                            File publicKeyFile = new File(String.format("%s.key",privateReceiver));
                            byte[] publicKeyBytes = Files.readAllBytes(publicKeyFile.toPath());
                            ...
                            System.out.println(thePublickey);
                            System.out.println("The key is : ");
                            // initializing the public key
                            //Cipher encryptCipher = Cipher.getInstance("RSA");
                            //encryptCipher.init(Cipher.ENCRYPT_MODE, thePublickey);
                            // encrypting the message
                            ChiffrementMisty1 m1 = new ChiffrementMisty1();
                            String theKey = "1001";
                            String TheMessage  = m1.Misty1(messageContent, clientName, privateReceiver,theKey);
                            System.out.println("the encrypted message");
                            System.out.println(TheMessage);
                            System.out.println("the encrypted message");
                            // encrypting the key with the public key
                            RSA RSACipher = new RSA();
                            String encryptedMisty1Key = RSACipher.encrypt("1001", thePublickey);
                            System.out.println("the encrypted key");
                            System.out.println(encryptedMisty1Key);
                            System.out.println("the encrypted key");
                            // create file for misty1 crypted key
                            FileWriter  outputFile = new FileWriter ("key/Misty1Key.txt");
                             outputFile.write(encryptedMisty1Key);
                             outputFile.close();
                            System.out.println("the encrypted key");
                            // adding the crypted misty1 key to the message so that it can be read by the reciever
                            
                            //sending the crypted message and the crypted key
...
*/
                             ...
                             try{
                                userDB.insertMessage(TheMessage,clientName, privateReceiver);
                            } catch(Exception e){
                                System.out.println(e.getMessage());
                            }
                            
                            sendToSpecificClient(st_receiver, "CMD_PRIVATECHAT|" + privateSender + "|" + TheMessage);
                            
                            System.out.println("[ServerThread] message = "+TheMessage);
                            break;
                        ...
                        case "CMD_ROOM":
                            ...
                            notifyToAllUsers("CMD_ONLINE_USERS|"+getAllUsers());
                            notifyToUsersInRoom("CMD_ONLINE_THIS_ROOM"+getUsersThisRoom());
                            notifyToUsersInRoom(clientName+" a rejoind le groupe");
                            break;
